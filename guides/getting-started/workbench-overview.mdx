...
Gamebooks are custom response workflows you build or agents can build by selecting actions for each affected entity.

### Step 1: Select an Entity

Left-click an entity icon in the Entity Context Graph. The atomic actions appear already mapped to the entity type selected. 
...
By click left-clicking entities, you will be able to utiliz the following atomic actions:

| Entity Type | Example Actions                                                         |
| ----------- | ----------------------------------------------------------------------- |
...
<Info>
  Available actions depend on the entity type and your connected modules. Gamebook actions automatically map to actions that are relevant and executable.
</Info>

### Step 3: Add Actions to Gamebook

- Click the **\+ icon** to add an action to your Gamebook
- Hover over the action in the Gamebook and the **red - icon** will remove an action
...
<Frame>
  ![Gamebook execution complete](/images/gamebook-execution-complete.png)
</Frame>
